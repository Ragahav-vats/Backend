...
exports.viewSubCategories = async(request, response) => {
    ...
    await subCategoryModel.find(filter)
    ...
    if(request.file){
     ...
     if(request.body != undefined){
        if(request.body.name != undefined || request.body.name != ''){
            ...
            saveData.slug = await generateUniqueSlug(subSubCategoryModel, slug);
        }
    }


    await subSubCategoryModel(saveData).save()
    .then((result) => {
        const data = {
        _status : true,
...
exports.view = async(request,response) => {
  ...
  var skip = 0;
   ...
   const andCondition = [
    ...
    var total_records = await subSubCategoryModel.find(filter).countDocuments();

  await subSubCategoryModel.find(filter).limit(limit).skip(skip)
  .select('name parent_category_id sub_category_id image slug status order')
