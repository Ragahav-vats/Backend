...
const {viewSubSubCategories, viewMaterials, viewColors,create, view, details, update, changeStatus, destroy } = require('../../controllers/admin/product.controllers');
const multer  = require('multer')
const uploads = multer({ dest: 'uploads/products' })
const path = require('path');
const { viewCategories, viewSubCategories } = require('../../controllers/admin/subSubCategory.controllers');
...
module.exports = server => {
    ...
    const uploadMiddleware = uploads.fields([{ name: 'image', maxCount: 1 }, { name: 'images', maxCount: 8 }])

   router.post('/view-sub-categories',upload.none(), viewSubCategories);
   router.post('/view-categories',upload.none(), viewCategories);


    router.post('/view-sub-sub-categories',uploads.none(), viewSubSubCategories);
