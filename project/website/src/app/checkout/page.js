...
export default function page() {
    ...
    const placeOrder = (event) => {
        ...
        axios.post(`${process.env.NEXT_PUBLIC_API_BASE_URL}/orders/place-order`,orderSave,
            {
            headers : {
    ...
    const razorpayInstance = new Razorpay(options);
     ...
     const orderStatusUpdate = (orderInfo) => {
                axios.post(`${process.env.NEXT_PUBLIC_API_BASE_URL}/orders/change-status`,orderInfo,
            {
            headers : {
                Authorization : `Bearer ${Cookies.get('token')}`
            }
