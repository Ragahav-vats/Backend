...
export default function ViewCategory() {
  ...
  useEffect(() => {
    axios.post(`${import.meta.env.VITE_API_BASE_URL}/${import.meta.env.VITE_SUB_SUB_CATEGORY}view`, {
      name : filterName,
  ...
  const changeStatus = () => {
    if (checkBoxValues.length > 0) {
      axios.post(`${import.meta.env.VITE_API_BASE_URL}/${import.meta.env.VITE_SUB_SUB_CATEGORY}change-status`, {
        ids: checkBoxValues
      })
        .then((result) => {
          if (result.data._status == true) {
  ...
  const deleteRecord = () => {
    if (checkBoxValues.length > 0) {
      ...
      if (confirm('Are you sure you want to delete ?')) {
        axios.post(`${import.meta.env.VITE_API_BASE_URL}/${import.meta.env.VITE_SUB_SUB_CATEGORY}destroy`, {
          ids: checkBoxValues
        })
          .then((result) => {
            if (result.data._status == true) {
