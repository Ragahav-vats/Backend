...
export default function ViewCategory() {
  ...
  useEffect(() => {
    axios.post(`${import.meta.env.VITE_API_BASE_URL}/${import.meta.env.VITE_CATEGORY}view`, {
      name: filterName,
      page: currentPage,
    })
      .then((result) => {
  ...
  const checkBoxValueAll = () => {
    if (categories.length == checkBoxValues.length) {
      setCheckBoxValues([]);
    } else {
      var checkBox = [];
  ...
  const changeStatus = () => {
    if (checkBoxValues.length > 0) {
      axios.post(`${import.meta.env.VITE_API_BASE_URL}/${import.meta.env.VITE_CATEGORY}change-status`, {
        ids: checkBoxValues
      })
        .then((result) => {
          if (result.data._status == true) {
  ...
  const deleteRecord = () => {
    if (checkBoxValues.length > 0) {
      ...
      if (confirm('Are you sure you want to delete ?')) {
        axios.post(`${import.meta.env.VITE_API_BASE_URL}/${import.meta.env.VITE_CATEGORY}destroy`, {
          ids: checkBoxValues
        })
          .then((result) => {
            if (result.data._status == true) {
  ...
  return (
    <section className="w-full">
      ...
      <div className={` rounded-lg border border-gray-300 px-5 py-5 max-w-[1220px] mx-auto mt-10 ${activeFilter ? "hidden" : "block"}`}>
        ...
        <form className="flex max-w-sm" onSubmit={filterCategory} autoComplete='off'>
                      ...
                      <th scope="col" class="p-4">
                        <div class="flex items-center">
                          <input id="checkbox-all-search"
                            ...
                            checked={
                              categories.length == checkBoxValues.length
                                ?
                                'checked'
                                :
                                ''
