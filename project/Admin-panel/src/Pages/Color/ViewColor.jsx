...
export default function ViewColor() {
    ...
    useEffect(() => {
        axios.post(`${ import.meta.env.VITE_API_BASE_URL }/${ import.meta.env.VITE_COLOR }view`, {
            name : filterName,
            code : filterCode,
            page : currentPage,
        })
    ...
    }, [filterName, currentPage, apiStatus, filterCode]);

    const filterColor = (e) => {
        e.preventDefault();
        setFilterName(e.target.name.value);
        setFilterCode(e.target.code.value);
        setCurrentPage(1)
    }
    ...
    const checkBoxValueAll = () => {
        if(colors.length == checkBoxValues.length){
            setCheckBoxValues([]);
        } else {
            var checkBox = [];
            colors.forEach((item) => {
    ...
    const changeStatus = () => {
        if(checkBoxValues.length > 0){
            axios.post(`${ import.meta.env.VITE_API_BASE_URL }/${ import.meta.env.VITE_COLOR }change-status`, {
                ids : checkBoxValues
            })
            .then((result) => {
                if(result.data._status == true){
    ...
    const deleteRecord = () => {
        if(checkBoxValues.length > 0){
            ...
            if(confirm('Are you sure you want to delete ?')){
                axios.post(`${ import.meta.env.VITE_API_BASE_URL }/${ import.meta.env.VITE_COLOR }destroy`, {
                    ids : checkBoxValues
                })
                .then((result) => {
                    if(result.data._status == true){
    ...
    return (
        <>
            <Breadcrumb path={"Color"} link={"/colors/view"} path2={"View"} slash={"/"} />
            <div className="w-full h-[610px]">
                <div className="max-w-[1220px] mx-auto py-2">
                    ...
                    <div className={` rounded-lg border border-gray-300 px-5 py-5 max-w-[1220px] mx-auto mt-10 ${activeFilter ? "hidden" : "block"}`}>
                        ...
                        <form className="flex max-w-lg" onSubmit={ filterColor } autoComplete='off'>
                                                ...
                                                <tr>
                                                    <th scope="col" class="p-4">
                                                        <div class="flex items-center">
                                                            ...
                                                            checked={
                                                                colors.length == checkBoxValues.length
                                                                ?
                                                                'checked'
                                                                :
                                                                ''
